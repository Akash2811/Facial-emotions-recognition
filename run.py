...
def emotion_analysis(emotions): 
    ...
    em_sort = sorted(emotions,reverse=True)
    expression = list(emotions)
    sort_exp=[]
    for i in range(7):
        if expression.index(max(expression)) == 0:
            val = expression.pop(0)
            sort_exp.append("Neutral")
        elif expression.index(max(expression)) == 1:
            val = expression.pop(1)
            sort_exp.append("Disgust")
        elif expression.index(max(expression)) == 2:
            val = expression.pop(2)
            sort_exp.append("Fear")
        elif expression.index(max(expression)) == 3:
            val = expression.pop(3)
            sort_exp.append("Sad")
        elif expression.index(max(expression)) == 4:
            val = expression.pop(4)
            sort_exp.append("Happy")
        elif expression.index(max(expression)) == 5:
            val = expression.pop(5)
            sort_exp.append("Surprised")
        elif expression.index(max(expression)) == 6:
            val = expression.pop(6)
            sort_exp.append("Angry")
    fig3 = go.Funnel(y=sort_exp,x=em_sort*100,name="",marker={'color' : 'tan'}, showlegend=False) #funnel
